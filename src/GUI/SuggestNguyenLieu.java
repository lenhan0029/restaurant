 ...
 * @author Shadow
 */
class SuggestNguyenLieu extends JDialog{
    private String maNL;
    private NguyenLieuBUS spBUS = new NguyenLieuBUS();
    private JTextField txtIDNL,txtTenNL,txtGia,txtSL;
    ...
    public SuggestNguyenLieu(String maNL)
    {
        this.maNL = maNL;
        setModal(true);
        init();
    }
    public SuggestNguyenLieu()
    {
        this.maNL ="";
        setModal(true);
        init();
    ...
    public void init()
    {
        setTitle("Danh sách Nguyên Liệu");
        setSize(DWIDTH,700);
...
/***************** PHẦN HIỂN THỊ THÔNG TIN ***************************/
        ...
        txtIDNL.setBounds(new Rectangle(120,20,250,30));
        itemView.add(lbMaNL);
        itemView.add(txtIDNL);
        
        JLabel lbTenNL = new JLabel("Tên nguyên liệu");
        lbTenNL.setFont(font0);
        lbTenNL.setBounds(20,70,100,30);
        txtTenNL = new JTextField();
        txtTenNL.setBounds(new Rectangle(120,70,250,30));
...
//        header.add("IMG");
        ...
        listSP();
        if (maNL.trim().length() != 0) {
            int select = searchModel(model, maNL);
...
//            data.add(s.getImg());
    ...
    {
        if(spBUS.getList()== null)spBUS.listNL();
        ArrayList<NguyenLieuDTO> nv = spBUS.getList();
        model.setRowCount(0);
        outModel(model,nv);
    }
    ...
    public void TabletoTXT(int i)
    {
        ...
        txtIDNL.setText(tbl.getModel().getValueAt(i, 0).toString());
        txtTenNL.setText(tbl.getModel().getValueAt(i, 1).toString());
